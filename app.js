...
async function initDatabase() {
    ...
    await db.exec(`CREATE TABLE IF NOT EXISTS qc_log (
        ...
        date TEXT NOT NULL,
        time TEXT NOT NULL,
        UNIQUE(username, patientID, sessionID, imageType)
    )`);
}
                  ...
                  for (const session of sessions) {
                      ...
                      try {
                          ...
                          if (anatStats.isDirectory()) {
                              ...
                              if ((imageType === 'T1' && hasT1) || (imageType === 'FLAIR' && hasFLAIR)) {
...
app.post('/log-qc', authenticate, async (req, res) => {
    const { patientID, sessionID, option, imageType, date, time } = req.body;
    const username = req.username;

    try {
        await db.run(`INSERT OR REPLACE INTO qc_log 
            (username, patientID, sessionID, option, imageType, date, time) 
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [username, patientID, sessionID, option, imageType, date, time]);
...
app.get('/get-qc-log', authenticate, async (req, res) => {
    const { patient, session, imageType } = req.query;
    const username = req.username;

    try {
        const row = await db.get(`SELECT option FROM qc_log 
            WHERE username = ? AND patientID = ? AND sessionID = ? AND imageType = ?`,
            [username, patient, session, imageType]);
        res.json(row || {});
    } catch (error) {
...
app.get('/database', authenticate, async (req, res) => {
    try {
        const rows = await db.all(`SELECT username, patientID, sessionID, option, imageType, date, time FROM qc_log`);
        res.json(rows);
    } catch (error) {
